...
void encontrarExtremos(DEQUE* d){
  int max;
  if (encontrarMax(d, &max)){
    printf("Maior chave: %i\n", max);
  }else{
    printf("Nao foi possivel encontrar a maior chave (DEQUE vazio).\n");
  }
}
...
void help(){
  ...
  printf("   x : encontrar a maior chave\n");
  printf("   q : sair\n");
}
...
int main(){
  ...
  while (comando != 'q'){
    switch (comando) {
      ...
      case 'x' : encontrarExtremos(&deque); break;
